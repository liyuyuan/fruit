...
public class RabbitConfig {
	...
	@Bean
	public Queue orderInfoCreate() {
		return new Queue("orderInfoCreate");
	}

	@Bean
	public SimpleMessageListenerContainer messageContainer() {
		/*
		 * Queue[] q = new Queue[queues.split(",").length]; for (int i = 0; i <
		 * queues.split(",").length; i++) { q[i] = new Queue(queues.split(",")[i]); }
		 */
